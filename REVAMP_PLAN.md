...
5. **Centralized XP config + duplicate-award guard** — single `XP` table; completion transaction IDs so toggling can't farm XP. (Duplicate-award guard verified — re-toggle 0→30→0, no double-award. Table centralization still pending.)
